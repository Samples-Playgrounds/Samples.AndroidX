	...
	{			
		...
		public static void Sample()
		{
			...
			try
			{
				...
				using (ElementWriter writer = new ElementWriter()) // ElementWriter is used to write Elements to the page
				{
					...
					doc.Save(Utils.CreateExternalFile("pdf_layers.pdf"), SDFDoc.SaveOptions.e_linearized);
			...
			try  
			{	 
				using (PDFDoc doc = new PDFDoc(Utils.CreateExternalFile("pdf_layers.pdf")))
				{
					...
					else 
					{
						...
						using (PDFDraw pdfdraw = new PDFDraw())
						{
							...
							pdfdraw.Export(page, Utils.CreateExternalFile("pdf_layers_default.png"));
							...
							if (ocgs != null) 
							{
								...
								for (i=0; i<sz; ++i) 
								{
									...
									pdfdraw.Export(page, Utils.CreateExternalFile(fname));
								}
							}
							...
							pdfdraw.Export(page, Utils.CreateExternalFile("pdf_layers_non_oc.png"));
		...
		static Obj CreateGroup1(PDFDoc doc, Obj layer)
		{
			...
			using (ElementBuilder builder = new ElementBuilder())
			{
				...
				Image img = Image.Create(doc.GetSDFDoc(), (Utils.GetAssetTempFile(input_path + "peppers.jpg")));
